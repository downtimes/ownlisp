...
//      interaction layer, add tail call optimization, type inference, lexical scoping
...
fn evaluate_eval(args: VecDeque<Ast>, env: &Rc<RefCell<Env>>) -> OwnlispResult {
    ...
    let qexpr = evaluate(args.pop_front().expect("We checked for size."), env)?;
...
fn evaluate_lambda(args: VecDeque<Ast>, env: &Rc<RefCell<Env>>) -> OwnlispResult {
    ...
    let mut qexprs = args
        ...
        .map(|expr| match expr {
            ...
            _ => bail!("Lambda only works with QExpressions!"),
        })
    ...
    let formals =
        qexprs
            ...
            .map(|symbol| match symbol {
                ...
                _ => bail!("Cannot define non-symbol"),
            });
...
fn evaluate_var(args: VecDeque<Ast>, env: &Rc<RefCell<Env>>, var_type: VarType) -> OwnlispResult {
    ...
    match args.pop_front() {
        ...
        Some(Ast::QExpression(names)) => {
            let names_str: VecDeque<_> = names
                .into_iter()
                .map(|ast| match ast {
                    Ast::Symbol(sym) => Ok(sym),
                    _ => bail!("Function def can not define non-symbol!"),
                })
                .collect::<Result<_, _>>()?;
            ...
            for (name, arg) in names_str.into_iter().zip(args.into_iter()) {
                match var_type {
                    VarType::Global => env.borrow_mut().put_global(name.clone(), arg),
                    VarType::Local => env.borrow_mut().put_local(name.clone(), arg),
                }
            }
            Ok(Ast::EmptyProgram)
        }
...
fn get_variadic_part(formals: &mut VecDeque<String>) -> Result<String> {
    let Some(rest_formal) = formals.pop_front() else {
        bail!("Function format invalid. Symbol ':' not followed by a single symbol!")
    };
...
fn evaluate_fun(
    ...
    while !arguments.is_empty() {
        let Some(formal) = formals.pop_front() else {
            bail!("Function passed too many arguments")
        };
    ...
    if !formals.is_empty() && formals[0] == ":" {
        ...
        let rest_formal = get_variadic_part(&mut formals)?;
        env.borrow_mut()
            .put_local(rest_formal, Ast::QExpression(VecDeque::new()));
    }
...
fn evaluate_sexpression(ast: Ast, env: &Rc<RefCell<Env>>) -> OwnlispResult {
    let Ast::SExpression(sexp) = ast else {
        panic!("called it wrong!")
    };
    ...
    if sexp.len() == 1 {
        ...
        match sexp.pop_front() {
            ...
            Some(Ast::Function(env, formals, body)) => evaluate_fun(env, formals, sexp, body),
...
fn main() {
    ...
    println!("Ownlisp version {version}");
