    ...
    fn clone(&self) -> Ast {
        match self {
            ...
            Ast::Function(env, form, body) => {
                ...
                let new_body = body.iter().cloned().collect();
                Ast::Function(new_env, form.clone(), new_body)
            }
            Ast::Symbol(s) => Ast::Symbol(s.clone()),
            Ast::SExpression(exp) => {
                let new_exp = exp.iter().cloned().collect();
...
impl PartialEq for Ast {
    fn eq(&self, rhs: &Ast) -> bool {
        match (self, rhs) {
            ...
            (Ast::SExpression(a), Ast::SExpression(b))
            | (Ast::QExpression(a), Ast::QExpression(b)) => a == b,
            (Ast::Symbol(a), Ast::Symbol(b)) | (Ast::Str(a), Ast::Str(b)) => a == b,
