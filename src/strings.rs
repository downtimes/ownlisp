...
pub(crate) fn print(args: VecDeque<Ast>, _env: &Rc<RefCell<Env>>) -> OwnlispResult {
    let mut args = args;
...
pub(crate) fn error(args: VecDeque<Ast>, _env: &Rc<RefCell<Env>>) -> OwnlispResult {
    let mut args = args;
    match args.pop_front() {
        Some(Ast::Str(s)) => {
            if !args.is_empty() {
...
pub(crate) fn load_ownlisp(args: VecDeque<Ast>, env: &Rc<RefCell<Env>>) -> OwnlispResult {
    let mut args = args;
    match args.pop_front() {
        Some(Ast::Str(s)) => {
            if !args.is_empty() {
